...
}:
{
  ...
  config = lib.mkIf config.themes.grub-lain.enable {
    ...
    boot.loader.grub = {
      ...
      efiSupport = true;
      efiInstallAsRemovable = config.themes.grub-lain.efiInstallAsRemovable;
      useOSProber = true;
    ...
    boot.loader.efi.canTouchEfiVariables = lib.mkForce (!config.themes.grub-lain.efiInstallAsRemovable);
  };
}
