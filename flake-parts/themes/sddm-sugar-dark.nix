...
let
  ...
  matugen-colors =
    pkgs.runCommand "matugen-sddm-colors"
      {
        nativeBuildInputs = [
          pkgs.matugen
          pkgs.jq
      ...
      ''
        ...
        HEX=$(magick "${cfg.background}[0]" -resize 1x1\! -format "%[hex:p{0,0}]" info:)
        COLORS=$(matugen color hex "$HEX" --json hex --dry-run --mode ${cfg.matugen.scheme} -q)
  ...
  themeColor =
    if cfg.matugen.enable && cfg.background != "" then
      builtins.readFile "${matugen-colors}/surface_color"
    else
      cfg.themeColor;

  accentColor =
    if cfg.matugen.enable && cfg.background != "" then
      builtins.readFile "${matugen-colors}/accent_color"
    else
      cfg.accentColor;
