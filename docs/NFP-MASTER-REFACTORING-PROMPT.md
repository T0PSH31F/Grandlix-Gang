...
**Source → Target:**
- `flake-parts/system/nix.nix` → `layers/00-foundation/nix-config.nix`
- `flake-parts/system/boot.nix` → `layers/00-foundation/boot.nix`
- `flake-parts/system/locale.nix` → `layers/00-foundation/locale.nix`
- `flake-parts/system/networking.nix` → `layers/00-foundation/networking.nix`

**Example:**
```nix
# layers/00-foundation/default.nix
{ ... }: {
  imports = [
    ./nix-config.nix
    ./boot.nix
...
# layers/00-foundation/nix-config.nix
{ ... }: {
  nix.settings = {
...
- `flake-parts/features/nixos/virtualization.nix` → Split into:
  ...
  - `layers/10-system/containers/podman.nix`
  - `layers/10-system/virtualization/qemu.nix`
  - `layers/10-system/virtualization/libvirt.nix`
...
**Source → Target:**
- `flake-parts/services/ai/` → `layers/20-services/ai/`
...
{ config, lib, pkgs, ... }: {
  imports = [
    ...
    ../../layers/10-system/containers/podman.nix
    ../../layers/20-services/web/caddy.nix
    ../../layers/20-services/databases/postgres.nix
    ../../layers/20-services/monitoring/prometheus.nix
  ];
...
{ config, lib, pkgs, ... }: {
  imports = [
    ...
    ../../layers/10-system/containers/podman.nix
    ../../layers/30-desktop
    ../../layers/40-applications/editors/vscode.nix
    ../../layers/40-applications/browsers/brave.nix
...
```bash
✓ All layers/ directories created
