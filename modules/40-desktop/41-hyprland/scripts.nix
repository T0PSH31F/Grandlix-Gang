...
let
  ...
  hypr-sfx = pkgs.writeShellScriptBin "hypr-sfx" ''
    ...
    SOUND_DIR="$HOME/Clan/NFP/modules/00-cyberia/02-assets/SFX"
  ...
  hypr-sfx-toggle = pkgs.writeShellScriptBin "hypr-sfx-toggle" ''
    ...
    if [ -f "$MUTE_FILE" ]; then
      ...
      ${pkgs.pipewire}/bin/pw-play "$HOME/Clan/NFP/modules/00-cyberia/02-assets/SFX/${cfg.sfx.sounds.openWindow}" &
    else
      touch "$MUTE_FILE"
      ${pkgs.pipewire}/bin/pw-play "$HOME/Clan/NFP/modules/00-cyberia/02-assets/SFX/${cfg.sfx.sounds.closeWindow}"
