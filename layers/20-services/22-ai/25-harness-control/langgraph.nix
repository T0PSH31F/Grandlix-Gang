...
let
  ...
  budgetConfig = pkgs.writeText "langgraph-budgets.json" (
    builtins.toJSON {
      # Budget tiers per agent/workflow
      # Kong route selection is driven by budget status
      agents = {
        default = {
      ...
      # Kong route mapping — budget status → Kong route
      routes = {
...
in
{
  options.services.ai-services.langgraph = {
    ...
    kongUrl = mkOption {
      type = types.str;
      default = "http://127.0.0.1:${toString config.services.ai-services.kong-gateway.proxyPort}";
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.langgraph = {
      ...
      environment = {
        ...
        # Kong Gateway as the sole LLM endpoint
