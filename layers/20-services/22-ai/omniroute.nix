...
# https://github.com/diegosouzapw/OmniRoute (fork: nqdev-fork/diegosouzapw-OmniRoute)
#
# Runs as an OCI container (upstream ships a Dockerfile; a Next.js/webpack
# build with 41 deps is too fragile for buildNpmPackage). Pattern mirrors
# the existing manifest.nix module: podman-compose + dataDir + env file.
{
  config,
  lib,
  pkgs,
...
let
  ...
  # Compose template — ${VAR} substitution happens at runtime from .env,
  # not from the Nix store, so secrets stay out of the store.
