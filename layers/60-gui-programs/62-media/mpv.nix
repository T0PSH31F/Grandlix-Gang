...
in
{
  ...
  home = { config, osConfig, ... }: lib.mkIf osConfig.layers.layer-60.gui.mpv.enable {
    xdg.desktopEntries.mvi = {
      ...
      mimeType = [ "image/jpeg" "image/png" "image/gif" "image/webp" "image/tiff" "image/svg+xml" ];
    };

    programs.mpv = {
      enable = true;
      scripts = with pkgs.mpvScripts; [
        webtorrent-mpv-hook mpris sponsorblock memo modernz thumbfast
        youtube-chat youtube-upnext autosubsync-mpv mpv-playlistmanager
      ];
      config = {
        profile = "gpu-hq";
        vo = "gpu";
    ...
    home.packages = with pkgs; [
      yt-dlp mpv-handler ff2mpv anime4k sickgear socat jq radiant-player
    ];
