...
- [ ] **Step 1: Add the evaluation registry to flake outputs**
          ...
          let
            ...
            extractMachineConfig = name: machine:
              let
                cfg = machine.config;
              in {
                # Extracting layer-20 services config state defensively
...
Run: `nix eval --json .#nfpuRegistry | jq .`
...
```bash
git add flake.nix
git commit -m "feat(nfpu): add nfpuRegistry to flake outputs"
```
...
- Create: `layers/00-cyberia/09-tools/nfpu/registry/dummy_registry.json`
