...
All modules live under `/layers/` using a **tiered decimal hierarchy**:
...
```nix
# Target State Example (Phase 11): layers/40-desktop/41-hyprland/default.nix
{ config, lib, pkgs, ... }: {
...
| Tool | Role |
|------|------|
| **`import-tree`** | Auto-scans `/layers/` and exports as flake outputs |
...
| Source | Target |
|--------|--------|
| `docs/` | `layers/00-cyberia/01-docs/` |
| `assets/` | `layers/00-cyberia/02-assets/` |
| `sops/` | `layers/00-cyberia/03-sops/` |
| `treasure/` | `layers/00-cyberia/04-treasure/` |
...
| Source | Target |
|--------|--------|
| `lib/` (new helpers) | `layers/80-lib/81-helpers/` |
| `overlays/` | `layers/80-lib/82-overlays/` |
| `packages/` (custom derivations) | `layers/80-lib/83-packages/` |
| `templates/` | `layers/80-lib/84-templates/` |
| `tests/` + `test-*.nix` | `layers/80-lib/85-tests/` |
| `scripts/` | `layers/80-lib/86-scripts/` |
| `iso/` | `layers/80-lib/87-iso/` |
...
|-------|------|-----------|
| **1** | Branch + create `/layers/` directory skeleton only | Visual check |
